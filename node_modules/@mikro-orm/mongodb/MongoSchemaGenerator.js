...
class MongoSchemaGenerator extends core_1.AbstractSchemaGenerator {
    ...
    mapIndexProperties(index, meta) {
        return core_1.Utils.flatten(core_1.Utils.asArray(index.properties).map(propName => {
    ...
    async executeQuery(collection, method, ...args) {
        ...
        return collection[method](...args).then((res) => {
            core_1.Utils.dropUndefinedProperties(args);
            const query = `db.getCollection('${collection.collectionName}').${method}(${args.map(arg => (0, node_util_1.inspect)(arg)).join(', ')});`;
